...
The shipped config runs `typed-jinja-lsp` from `PATH`, which works when the project venv is active. For a project-local venv without activation, either point `cmd` at the absolute path (`<root>/.venv/bin/typed-jinja-lsp`), use the `uv run` form (`cmd = { 'uv', 'run', 'typed-jinja-lsp' }`), or use the venv-resolving function shown in the comments of `typed_jinja.lua`.

## What it checks

The server checks the file on disk on open and on save (not unsaved buffer edits, for now). It runs the same checker as the CLI, so diagnostics match `typed-jinja check`.
